...
function setupNotes() {
  ...
  const api = {
    ...
    append(text, meta = {}) {
      ...
      const block = [meta.title ? `[${meta.title}]` : `[${title}]`, String(text).trim()].filter(Boolean).join('\n');
    ...
    appendMany(items, meta = {}) {
      ...
      const blocks = cleaned.map(item => [meta.title ? `[${meta.title}]` : `[${title}]`, item].join('\n')).join('\n\n— — —\n\n');
      const sep = textarea.value.trim() ? '\n\n— — —\n\n' : '';
...
function initStudyHighlighter(root) {
  ...
  const getTextNodes = () => {
    const walker = document.createTreeWalker(root, NodeFilter.SHOW_TEXT, {
      acceptNode(node) {
        ...
        if (node.parentElement?.closest('.notes-card, .snapshot-card, .panel, .study-mobile-bar, .selection-status')) return NodeFilter.FILTER_REJECT;
  ...
  const rerender = () => {
    ...
    items.forEach(item => {
      const range = rangeFromOffsets(item.start, item.end);
      if (range) wrapRange(range, item);
    });
    updatePendingUI(items);
  };

  const selectionInsideRoot = () => {
  ...
  const rememberSelection = () => {
    const selection = window.getSelection();
    if (!selection || selection.isCollapsed || !selection.rangeCount || !selectionInsideRoot()) return;
    const range = selection.getRangeAt(0).cloneRange();
    const offsets = offsetsFromRange(range);
    if (offsets?.text) lastSelection = offsets;
  };

  const addHighlightFromLastSelection = () => {
    if (!lastSelection || !lastSelection.text) {
      status('Prima seleziona un passaggio del testo');
      return;
    }
    const items = loadHighlights();
    const duplicate = items.some(item => item.start === lastSelection.start && item.end === lastSelection.end);
    if (duplicate) {
      status('Questo passaggio è già evidenziato');
      return;
    }
    const overlap = items.some(item => !(lastSelection.end <= item.start || lastSelection.start >= item.end));
    if (overlap) {
      status('La selezione si sovrappone a un’evidenziazione già presente');
      return;
    }
    items.push({
  ...
  const savePendingHighlights = () => {
    ...
    window.FOSCOLO_NOTES_API?.appendMany?.(pending.map(item => item.text), { title: document.body.dataset.title || 'Lezione' });
    const now = Date.now();
  ...
  document.addEventListener('selectionchange', () => {
    if (selectionInsideRoot()) rememberSelection();
  });
