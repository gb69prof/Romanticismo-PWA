...
function setupNotes() {
  ...
  let saveHighlightsBtn = box.querySelector('[data-action="save-highlights-batch"]');
  if (!saveHighlightsBtn) {
    saveHighlightsBtn = document.createElement('button');
    saveHighlightsBtn.className = 'btn secondary';
    saveHighlightsBtn.type = 'button';
    saveHighlightsBtn.dataset.action = 'save-highlights-batch';
    saveHighlightsBtn.innerHTML = 'Salva evidenziazioni negli appunti <span class="notes-badge" data-highlights-pending-count hidden>0</span>';
    const actions = box.querySelector('.notes-actions');
    actions?.insertBefore(saveHighlightsBtn, clearBtn || null);
  }
...
${textarea?.value || ''}`;
  ...
  api.batchButton = saveHighlightsBtn;
  api.updatePendingCount = (count) => {
    if (!saveHighlightsBtn) return;
    const badge = saveHighlightsBtn.querySelector('[data-highlights-pending-count]');
    if (badge) {
      badge.hidden = !count;
      badge.textContent = String(count || 0);
    }
    saveHighlightsBtn.disabled = !count;
    saveHighlightsBtn.classList.toggle('is-disabled', !count);
  };
...
function initStudyHighlighter(root) {
  ...
  const wrapRange = (range, id, savedToNotes = false) => {
    ...
    nodes.forEach(node => {
      ...
      if (savedToNotes) mark.classList.add('saved-to-notes');
      mark.dataset.highlightId = id;
      mark.dataset.savedToNotes = savedToNotes ? '1' : '0';
      mark.textContent = middle;
      mark.title = savedToNotes ? 'Passaggio evidenziato e già salvato negli appunti' : 'Passaggio evidenziato';
  ...
  const rerenderHighlights = () => {
    ...
    items.forEach(item => {
      ...
      if (range) wrapRange(range, item.id, !!item.savedToNotes);
    });
    updatePendingHighlightsUI(items);
  };
  ...
  const addHighlight = (selectionData, alsoAddNote) => {
    ...
    items.push({
      ...
      createdAt: Date.now(),
      savedToNotes: !!alsoAddNote,
      savedAt: alsoAddNote ? Date.now() : null
    });
  ...
  const appendManyToNotes = (texts) => {
    const cleaned = (texts || []).map(item => String(item || '').trim()).filter(Boolean);
    if (!cleaned.length) return false;
    const api = window.FOSCOLO_NOTES_API;
    if (api?.appendMany) {
      api.appendMany(cleaned, { title: document.body.dataset.title || 'Lezione' });
      return true;
    }
    cleaned.forEach(text => appendToNotes(text));
    return true;
  };

  const updatePendingHighlightsUI = (items = loadHighlights()) => {
    const pending = items.filter(item => !item.savedToNotes).length;
    const api = window.FOSCOLO_NOTES_API;
    api?.updatePendingCount?.(pending);
  };

  const savePendingHighlightsToNotes = () => {
    const items = loadHighlights();
    const pending = items.filter(item => !item.savedToNotes);
    if (!pending.length) {
      showNoteStatus('Non ci sono nuove evidenziazioni da salvare negli appunti');
      updatePendingHighlightsUI(items);
      return;
    }
    appendManyToNotes(pending.map(item => item.text));
    ...
    saveHighlights(items);
    rerenderHighlights();
    showNoteStatus(pending.length === 1 ? '1 evidenziazione salvata negli appunti' : `${pending.length} evidenziazioni salvate negli appunti`);
  };
  ...
  window.addEventListener('foscolo:save-pending-highlights', savePendingHighlightsToNotes);
  rerenderHighlights();
}
